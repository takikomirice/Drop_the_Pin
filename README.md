...
- 共有ビューでのルート一覧、ルート別表示切り替え、ルート外ピン表示制御、ルート表示初期化
...
- `shared.html`
  ...
  - 表示中ルートとルート外ピン表示設定を含めた共有ビュー内フィルタリング
  - ルート表示とルート外ピン設定の初期化
  - 道路ルート / 直線ルートの表示切り替え
...
次の機能は、検索 / フィルタ / 並び替え・タグ / 状態管理・一括操作・ルート管理・共有ビューのルート一覧と表示制御・キャッシュ済み道路ルート表示が導入済みとなった現時点では、後続フェーズの検討候補として残っています。
