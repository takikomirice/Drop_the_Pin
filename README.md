...
- `Code.js`
  ...
  - `createShareLink()` / `listShareLinks()` / `getSharedViewData()`: 共有リンク作成、管理、共有ビュー用データ取得
...
ローカルでは、Node.js の組み込み test runner で UI ガードの静的テストを実行できます。

```bash
node --test tests/route-view-mode.test.mjs
```
...
次の機能は、検索 / フィルタ / 並び替え・タグ / 状態管理・一括操作・ルート管理が導入済みとなった現時点では、後続フェーズの検討候補として残っています。
